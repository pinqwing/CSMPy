...
Already simple models can be compiled and run. Working integration methods now include RECT, TRAPZ, SIMPSON and RKSFX. I am currently working on the variable timestep methods. 
Keyword and functions support still is incomplete. Print statements are executed immediately rather than at the end of the run. This puts a limit to the number of variables that can still be printed in a readable manner.
...
Since CSMPy uses Abstract Syntax Trees rather than line parsing, it is expected that most if not all python statements will be sorted as black boxes, without requiring NOSORT blocks or interrupting SORT blocks and splitting them in multiple independent blocks, as CSMP did. (As of today, this has not yet been well tested).
